...
# Check whether a local .spectrally/ repo exists
...
_CUSTOM = os.path.join(_CUSTOM, 'scripts', 'spectrallycustom.py')
...
def _get_PATH_LOCAL():
    pfe = os.path.join(os.path.expanduser('~'), '.spectrally', 'openadas')
...
def step01_search_online(
    ...
    example
    -------
        >>> import spectrally as tf
        >>> tf.openadas.step01_search_online('ar+16 ADF15')
    """
...
def step01_search_online_by_wavelengthA(
    ...
    -------
        >>> import spectrally as tf
        >>> tf.openadas.step01_earch_online_by_wavelengthA(3., 4., 'ar')
        >>> tf.openadas.step01_earch_online_by_wavelengthA(
            3., 4., 'w',
            resolveby='file',
        )
    """
...
def _check_exists(filename, update=None, create_custom=None):
    ...
    # Check whether the local .spectrally repo exists
    # if not recommend spectrally-custom
    if path_local is None:
        path = os.path.join(os.path.expanduser('~'), '.spectrally', 'openadas')
        ...
        if path_local is None or create_custom is False:
            msg = (
                "You do not seem to have a local .spectrally repository\n"
                "spectrally uses that local repository to store user-specific "
                "data and downloads\n"
                "In particular, openadas files are downloaded in:\n"
                + f"\t{path}\n"
                "  => to set-up your local .spectrally repo, run in terminal:\n"
                "\tspectrally custom"
            )
            raise Exception(msg)
...
def step02_download(
    ...
    All downloaded files are stored in your local spectrally directory
    (~/.spectrally/)

    Automatically runs spectrally-custom if create_custom=True

    example
    -------
        >>> import spectrally as sp
        >>> filename = '/adf15/pec40][ar/pec40][ar_ls][ar16.dat'
        >>> sp.openadas.step02_download(filename)
    """
...
def step02_download_all(
    ...
    -------
        >>> import spectrally as sp
        >>> sp.openadas.step02_download_all(
...
def clear_downloads():
    """ Delete all openadas files downloaded in your ~/.spectrally/ directory """
    path_local = _get_PATH_LOCAL()
    if path_local is None:
        return
    lf = [
