...
def _get_PATH_LOCAL(strict=True):
    pfe = os.path.join(os.path.expanduser('~'), f'.{_PKG}', 'nist')
...
def _csv_parser(
    ...
    if url is not None:
        if path_local is None:
            if create_custom is True:
                ...
                path = os.path.join(
                    os.path.expanduser('~'), f'.{_PKG}', 'nist',
                )
                msg = (
                    f"You do not seem to have a local ./{_PKG} repository\n"
                    f"{_PKG} uses that local repository to store user-specific "
                    "data and downloads\n"
                    "In particular, nist files are downloaded in:\n"
                    f"\t{path}\n"
                    f"  => to set-up your local .{_PKG} repo, run (terminal):\n"
                    f"\t{_PKG} custom"
                )
                raise Exception(msg)
    ...
    with open(pfe, mode='r') as csv_file:
        ...
        line_count = 0
        for i0, row in enumerate(csv_reader):
            if line_count == 0:
                line_count += 1
            else:
                # remove useless char
                rowi = {}
                for i1, (k0, v0) in enumerate(row.items()):
                    if k0 not in lkout:
...
def step01_search_online_by_wavelengthA(
    ...
    -------
        ...
        >>> sp.nist.step01_search_online_by_wavelengthA(
            lambmin=3.94,
            lambmax=4,
            ion='ar',
        )
...
def clear_cache():
    f""" Delete all nist files downloaded in your ~/.{_PKG}/ directory """
